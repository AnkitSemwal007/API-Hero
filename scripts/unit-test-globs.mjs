/**
 * Shared unit-test globs for `npm test` and `npm run test:coverage`.
 * Keep this list as the single source of truth.
 */
export const UNIT_TEST_GLOBS = Object.freeze([
  'dist/auth/*.test.js',
  'dist/auth/ui/*.test.js',
  'dist/auth/vscode/*.test.js',
  'dist/assertions/*.test.js',
  'dist/extraction/*.test.js',
  'dist/extraction/shared/*.test.js',
  'dist/extraction/extractors/*.test.js',
  'dist/extraction/vscode/*.test.js',

  'dist/commands/*.test.js',
  'dist/constants/*.test.js',
  'dist/migration/*.test.js',
  'dist/collections/*.test.js',
  'dist/collections/mutation/*.test.js',
  'dist/collections/transfer/*.test.js',
  'dist/collections/vscode/*.test.js',
  'dist/collection-runner/*.test.js',
  'dist/collection-runner/vscode/*.test.js',
  'dist/scenarios/*.test.js',
  'dist/scenarios/graph/*.test.js',
  'dist/scenarios/templates/*.test.js',
  'dist/scenarios/ui-model/*.test.js',
  'dist/scenarios/validation/*.test.js',
  'dist/scenarios/variables/*.test.js',
  'dist/scenarios/execution/*.test.js',
  'dist/scenarios/execution/step-runners/*.test.js',
  'dist/scenarios/storage/*.test.js',
  'dist/scenarios/serialization/*.test.js',
  'dist/scenarios/report/*.test.js',
  'dist/scenarios/vscode/*.test.js',
  'dist/dependencies/*.test.js',
  'dist/regression/*.test.js',
  'dist/history/*.test.js',
  'dist/history/vscode/*.test.js',
  'dist/language-support/core/*.test.js',
  'dist/openapi-import/*.test.js',
  'dist/openapi-import/generators/*.test.js',
  'dist/openapi-import/postman/*.test.js',
  'dist/openapi-import/insomnia/*.test.js',
  'dist/openapi-import/providers/*.test.js',
  'dist/openapi-import/vscode/*.test.js',
  'dist/overview/vscode/*.test.js',
  'dist/project-store/*.test.js',
  'dist/project-store/vscode/*.test.js',
  'dist/project-package/*.test.js',
  'dist/parser/ast/*.test.js',
  'dist/parser/tokenizer/*.test.js',
  'dist/parser/lexer/*.test.js',
  'dist/parser/*.test.js',
  'dist/request/*.test.js',
  'dist/request-source/*.test.js',
  'dist/source-integration/*.test.js',
  'dist/request-editor/vscode/*.test.js',
  'dist/execution/*.test.js',
  'dist/orchestration/*.test.js',
  'dist/curl/*.test.js',
  'dist/codegen/*.test.js',
  'dist/response/*.test.js',
  'dist/variables/*.test.js',
  'dist/variables/vscode/*.test.js',
  'dist/storage/*.test.js',
  'dist/shared/*.test.js',
  'dist/ui/webview/*.test.js',
  'dist/ui/webview/icons/*.test.js',
  'dist/mcp/*.test.js',
  'dist/cli/*.test.js',
  'dist/headless/*.test.js',
]);
